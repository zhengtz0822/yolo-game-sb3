...
CUSTOM_ACTIONS = [
    ...
    24,  # 3:  右跳                   = (0, RIGHT, A)
    9,   # 4:  射击                   = (0, 0, B)
    15,  # 5:  右走+射击              = (0, RIGHT, B)
    33,  # 6:  右跳+射击              = (0, RIGHT, B+A)
]
...
# 含跳跃的动作索引（用于跳跃奖励判断）
JUMP_ACTIONS = {2, 3}              # 纯跳跃动作（跳、右跳）
JUMP_RIGHT_ACTIONS = {3, 6}        # 跳跃+右移动作（右跳、右跳+射击）

# ============ 完整动作集（15个，retrain 阶段使用） ============
# CUSTOM_ACTIONS = [
#     0,   # 0:  不动 (NOOP)            = (0, 0, 0)
#     6,   # 1:  右走                   = (0, RIGHT, 0)
#     3,   # 2:  左走                   = (0, LEFT, 0)
